class('NotchEncoder').extends(playdate.graphics.sprite)

-- TODO - multi select notches - for LFO types
-- A knob. Parent must import Coracle/coracle and Coracle/math
function NotchEncoder:init(x, y, listener, _font)
	NotchEncoder.super.init(self)
...
function NotchEncoder:removeChildren()
	self.backplate:remove()
	self.labelSprite:remove()
	self.focusedSprite:remove()
end

function NotchEncoder:addChildren()
	self.backplate:add()
	self.labelSprite:add()
	self.focusedSprite:add()
end
...
-- 0.0 to 1.0
function NotchEncoder:getValue()
	return map(self:getRotation(), 0, 300, 0.0, 1.0)
end

-- 0.0 to 1.0
function NotchEncoder:setValue(value)
...
function NotchEncoder:turn(degrees)
...
function NotchEncoder:setFocus(focus)
	self.hasFocus = focus
	self.focusedSprite:setVisible(focus)
	self:update()
end

function NotchEncoder:focused()
	return self.hasFocus
end


function NotchEncoder:updateLabel()
...
function NotchEncoder:getLabel()
